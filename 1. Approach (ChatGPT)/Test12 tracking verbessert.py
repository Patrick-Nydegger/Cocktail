...
def ocr_recognition(crop_img, lang='eng', save_dir=r'C:\Users\padin\Downloads\img'):
    try:
        ...
        image_path = os.path.join(save_dir, 'bild.png')      # Du kannst hier einen eindeutigen Namen generieren, wenn nötig
...
CONFIDENCE_THRESHOLD = 0.3  # Mindestvertrauensschwelle für Verifizierung
...
# Zähler zur Verifikation der Tracker
    ...
    for result in results[0].boxes:
        ...
        if class_id == 39 and confidence > 0.7:  # Nur Flaschen erkennen
            ...
            if not found_existing_tracker:
                ...
                # Füge das neue Label zu bottle_instances hinzu
                bottle_instances[new_label] = []  # Erstellt einen leeren OCR-Text-Eintrag für die Flasche
    ...
    for i, tracker in enumerate(trackers):
        ...
        if success:
            ...
            if recognized_text:
                ...
                # Speichere erkannte Texte sofort in bottle_instances
                bottle_instances[tracker_labels[i]].append(recognized_text)
            ...
            active_labels.append(tracker_labels[i])
            active_ocr_lists.append(tracker_ocr_lists[i])

        else:
            # Wenn der Tracker nicht mehr erfolgreich ist, keine Aktion nötig, OCR wurde bereits gespeichert

            print(f"Tracker {tracker_labels[i]} ist nicht mehr gültig, wird aber nicht gelöscht. OCR wurde bereits gespeichert.")

    # Aktualisiere die Tracker-, Label- und OCR-Listen
    trackers = active_trackers
    tracker_labels = active_labels
    tracker_ocr_lists = active_ocr_lists

    # Regelmäßige Überprüfung der Tracker durch YOLO alle 10 Frames
    if frame_count % verification_interval == 0:
        for i, tracker in enumerate(trackers):
            (x, y, w, h) = [int(v) for v in bboxes[i]]
            roi = frame[y:y + h, x:x + w]
            verify_results = yolo(roi)

            # Überprüfen, ob die Verifikation erfolgreich ist (confidence > 0.3)
            if len(verify_results[0].boxes) == 0 or float(verify_results[0].boxes[0].conf.cpu().numpy()) < CONFIDENCE_THRESHOLD:
                print(f"Tracker {tracker_labels[i]} nicht mehr gültig und wird entfernt.")
                trackers.pop(i)
                tracker_labels.pop(i)
                tracker_ocr_lists.pop(i)
